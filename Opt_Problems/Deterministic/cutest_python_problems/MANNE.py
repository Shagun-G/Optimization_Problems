...
class MANNE(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        if "K1" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["K1"]), float(3.05)
            )
        for I in range(int(v_["2"]), int(v_["T"]) + 1):
            v_["I-1"] = -1 + I
            ...
            if "K" + str(I) in ix_:
                ...
                self.y0 = arrset(
                    self.y0,
                    findfirst(self.congrps, lambda x: x == ig_["K" + str(I)]),
                    float(v_["VAL"]),
                )
        for I in range(int(v_["1"]), int(v_["T"]) + 1):
            if "C" + str(I) in ix_:
                ...
                self.y0 = arrset(
                    self.y0,
                    findfirst(self.congrps, lambda x: x == ig_["C" + str(I)]),
                    float(0.95),
                )
            if "I" + str(I) in ix_:
                ...
                self.y0 = arrset(
                    self.y0,
                    findfirst(self.congrps, lambda x: x == ig_["I" + str(I)]),
                    float(0.05),
                )
        # %%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_ = {}
