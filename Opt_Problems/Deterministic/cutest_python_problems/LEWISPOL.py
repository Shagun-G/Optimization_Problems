    ...
    def __init__(self, *args):
        ...
        for I in range(int(v_["1"]), int(v_["DEG-1"]) + 1):
            ...
            for J in range(int(I), int(v_["N-1"]) + 1):
                v_["RJ"] = float(J)
                v_["C" + str(I) + "," + str(J)] = (
                    v_["C" + str(int(v_["I-1"])) + "," + str(J)] * v_["RJ"]
                )
        ...
        self.gconst = arrset(
            self.gconst, ig_["D0"], float(v_["CT" + str(int(v_["0"]))])
        )
        for I in range(int(v_["1"]), int(v_["DEG-1"]) + 1):
            v_["I-1"] = -1 + I
            v_["-I"] = -1 * I
            ...
            self.gconst = arrset(
                self.gconst, ig_["D" + str(I)], float(v_["CT" + str(I)])
            )
        # %%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        if "A0" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["A0"]), float(-1.0)
            )
        if "A1" in ix_:
            self.x0[ix_["A1"]] = float(1.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["A1"]), float(1.0)
            )
        if "A2" in ix_:
            self.x0[ix_["A2"]] = float(1.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["A2"]), float(1.0)
            )
        if "A3" in ix_:
            self.x0[ix_["A3"]] = float(0.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["A3"]), float(0.0)
            )
        if "A4" in ix_:
            self.x0[ix_["A4"]] = float(1.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["A4"]), float(1.0)
            )
        if "A5" in ix_:
            self.x0[ix_["A5"]] = float(-1.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["A5"]), float(-1.0)
            )
        # %%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_ = {}
        elftv = []
