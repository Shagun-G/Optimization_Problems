...
class HS106(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        if "X2" in ix_:
            ...
            self.y0 = arrset(
                self.y0, np.where(self.congrps == ig_["X2"])[0], float(5000.0)
            )
        if "X3" in ix_:
        ...
        if "X4" in ix_:
            ...
            self.y0 = arrset(
                self.y0, np.where(self.congrps == ig_["X4"])[0], float(200.0)
            )
        if "X5" in ix_:
            self.x0[ix_["X5"]] = float(350.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X5"]), float(350.0)
            )
        if "X6" in ix_:
            self.x0[ix_["X6"]] = float(150.0)
        else:
            self.y0 = arrset(
                self.y0, np.where(self.congrps == ig_["X6"])[0], float(150.0)
            )
        if "X7" in ix_:
            self.x0[ix_["X7"]] = float(225.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X7"]), float(225.0)
            )
        if "X8" in ix_:
            self.x0[ix_["X8"]] = float(425.0)
        else:
            self.y0 = arrset(
                self.y0, np.where(self.congrps == ig_["X8"])[0], float(425.0)
            )
        # %%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_ = {}
        elftv = []
