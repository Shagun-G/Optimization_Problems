...
class KOWOSBNE(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        if "X1" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X1"]), float(0.25)
            )
        if "X2" in ix_:
            self.x0[ix_["X2"]] = float(0.39)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X2"]), float(0.39)
            )
        if "X3" in ix_:
            self.x0[ix_["X3"]] = float(0.415)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X3"]), float(0.415)
            )
        if "X4" in ix_:
            self.x0[ix_["X4"]] = float(0.39)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X4"]), float(0.39)
            )
        # %%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_ = {}
        elftv = []
