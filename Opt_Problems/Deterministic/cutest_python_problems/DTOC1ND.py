...
class DTOC1ND(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        for T in range(int(v_["1"]), int(v_["N-1"]) + 1):
            ...
            irA = np.append(irA, [ig])
            icA = np.append(
                icA, [ix_["Y" + str(int(v_["T+1"])) + "," + str(int(v_["1"]))]]
            )
            valA = np.append(valA, float(-1.0))
            [ig, ig_, _] = s2mpj_ii("TT" + str(T) + "," + str(int(v_["1"])), ig_)
            gtype = arrset(gtype, ig, "==")
            ...
            for I in range(int(v_["1"]), int(v_["NX"]) + 1):
                ...
                icA = np.append(icA, [ix_["X" + str(T) + "," + str(I)]])
                valA = np.append(
                    valA, float(v_["B" + str(int(v_["1"])) + "," + str(I)])
                )
            ...
            irA = np.append(irA, [ig])
            icA = np.append(
                icA, [ix_["Y" + str(int(v_["T+1"])) + "," + str(int(v_["NY"]))]]
            )
            valA = np.append(valA, float(-1.0))
            [ig, ig_, _] = s2mpj_ii("TT" + str(T) + "," + str(int(v_["NY"])), ig_)
            gtype = arrset(gtype, ig, "==")
            ...
            for I in range(int(v_["1"]), int(v_["NX"]) + 1):
                ...
                icA = np.append(icA, [ix_["X" + str(T) + "," + str(I)]])
                valA = np.append(
                    valA, float(v_["B" + str(int(v_["NY"])) + "," + str(I)])
                )
        # %%%%%%%%%%%%%% GLOBAL DIMENSIONS %%%%%%%%%%%%%%%%%
        self.n = len(ix_)
        ngrp = len(ig_)
        ...
        for T in range(int(v_["1"]), int(v_["N-1"]) + 1):
            for I in range(int(v_["1"]), int(v_["NX"]) + 1):
                self.gconst = arrset(
                    self.gconst, ig_["OX" + str(T) + "," + str(I)], float(-0.5)
                )
        for T in range(int(v_["1"]), int(v_["N"]) + 1):
            for I in range(int(v_["1"]), int(v_["NY"]) + 1):
                self.gconst = arrset(
                    self.gconst, ig_["OY" + str(T) + "," + str(I)], float(-0.25)
                )
        # %%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        for T in range(int(v_["1"]), int(v_["N-1"]) + 1):
            for K in range(int(v_["0"]), int(v_["NYNX-1"]) + 1):
                ...
                self.elpar = loaset(
                    ...
                    ie,
                    posep[0],
                    float(v_["C" + str(int(v_["I"])) + "," + str(int(v_["J"]))]),
                )
        # %%%%%%%%%%%%%%%%%%%%% GRFTYPE %%%%%%%%%%%%%%%%%%%%
        igt_ = {}
        ...
        for T in range(int(v_["1"]), int(v_["N-1"]) + 1):
            for J in range(int(v_["1"]), int(v_["NY"]) + 1):
                for K in range(int(v_["0"]), int(v_["NYNX-1"]) + 1):
                    ...
                    self.grelt = loaset(
                        self.grelt, ig, posel, ie_["E" + str(T) + "," + str(K)]
                    )
