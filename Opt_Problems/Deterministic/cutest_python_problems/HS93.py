...
class HS93(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        self.gconst = arrset(self.gconst, ig_["C1"], float(2.07e0))
        self.gconst = arrset(self.gconst, ig_["C2"], float(1.0e0))
        # %%%%%%%%%%%%%%%%%%% START POINT %%%%%%%%%%%%%%%%%%
        ...
        if "X1" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X1"]), float(5.54)
            )
        if "X2" in ix_:
            self.x0[ix_["X2"]] = float(4.4)
        else:
            self.y0 = arrset(
                self.y0, np.where(self.congrps == ig_["X2"])[0], float(4.4)
            )
        if "X3" in ix_:
            self.x0[ix_["X3"]] = float(12.02)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X3"]), float(12.02)
            )
        if "X4" in ix_:
            self.x0[ix_["X4"]] = float(11.82)
        else:
            self.y0 = arrset(
                self.y0, np.where(self.congrps == ig_["X4"])[0], float(11.82)
            )
        if "X5" in ix_:
            self.x0[ix_["X5"]] = float(0.702)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X5"]), float(0.702)
            )
        if "X6" in ix_:
            self.x0[ix_["X6"]] = float(0.852)
        else:
            self.y0 = arrset(
                self.y0, np.where(self.congrps == ig_["X6"])[0], float(0.852)
            )
        pass
        # %%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_ = {}
    ...
    def eOE3(self, nargout, *args):
        ...
        if nargout > 1:
            ...
            g_[2] = IV_[0] * IV_[1] * 2.0e0 * IV_[2] * IV_[3]
            ...
            if nargout > 2:
                ...
                H_[1, 0] = H_[0, 1]
                H_[0, 2] = IV_[1] * 2.0e0 * IV_[2] * IV_[3]
                H_[2, 0] = H_[0, 2]
                H_[0, 3] = IV_[1] * (IV_[2] ** 2)
                H_[3, 0] = H_[0, 3]
                H_[1, 2] = IV_[0] * 2.0e0 * IV_[2] * IV_[3]
                H_[2, 1] = H_[1, 2]
                H_[1, 3] = IV_[0] * (IV_[2] ** 2)
                H_[3, 1] = H_[1, 3]
                H_[2, 2] = IV_[0] * IV_[1] * 2.0e0 * IV_[3]
                H_[2, 3] = IV_[0] * IV_[1] * 2.0e0 * IV_[2]
                H_[3, 2] = H_[2, 3]
                H_ = U_.T.dot(H_).dot(U_)
        if nargout == 1:
            return f_
