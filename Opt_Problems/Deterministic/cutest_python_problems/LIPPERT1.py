...
class LIPPERT1(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        for I in range(int(v_["1"]), int(v_["NX"]) + 1):
            for J in range(int(v_["1"]), int(v_["NY"]) + 1):
                self.gconst = arrset(
                    self.gconst, ig_["A" + str(I) + "," + str(J)], float(1.0)
                )
                self.gconst = arrset(
                    self.gconst, ig_["B" + str(I) + "," + str(J)], float(1.0)
                )
                self.gconst = arrset(
                    self.gconst, ig_["C" + str(I) + "," + str(J)], float(1.0)
                )
                self.gconst = arrset(
                    self.gconst, ig_["D" + str(I) + "," + str(J)], float(1.0)
                )
        # %%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        for I in range(int(v_["1"]), int(v_["NX"]) + 1):
            ...
            for J in range(int(v_["1"]), int(v_["NY"]) + 1):
                ...
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["P" + str(int(v_["I-1"])) + "," + str(J)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, 1.0)
                posel = len(self.grelt[ig])
                ...
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["Q" + str(I) + "," + str(int(v_["J-1"]))]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, 1.0)
