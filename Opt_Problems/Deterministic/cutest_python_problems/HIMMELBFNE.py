...
class HIMMELBFNE(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        if "X1" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X1"]), float(2.7)
            )
        if "X2" in ix_:
            self.x0[ix_["X2"]] = float(90.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X2"]), float(90.0)
            )
        if "X3" in ix_:
        ...
        if "X4" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X4"]), float(10.0)
            )
        # %%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_ = {}
        elftv = []
    ...
    def eHF(self, nargout, *args):
        ...
        U = (
            EV_[0] * EV_[0]
            + self.elpar[iel_][0] * EV_[1] * EV_[1]
            + self.elpar[iel_][0] * self.elpar[iel_][0] * EV_[2] * EV_[2]
        )
