...
class POROUS1(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        self.gconst = arrset(
            self.gconst,
            ig_["G" + str(int(v_["P-1"])) + "," + str(int(v_["P-1"]))],
            float(-50.0),
        )
        # %%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        for I in range(int(v_["2"]), int(v_["P-1"]) + 1):
            ...
            for J in range(int(v_["2"]), int(v_["P-1"]) + 1):
                ...
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["US" + str(I) + "," + str(J)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
