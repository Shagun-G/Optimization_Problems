...
class LSNNODOC(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        if "X1" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X1"]), float(4.0)
            )
        if "X2" in ix_:
            self.x0[ix_["X2"]] = float(6.0)
        else:
            self.y0 = arrset(
                self.y0, np.where(self.congrps == ig_["X2"])[0], float(6.0)
            )
        if "X3" in ix_:
        ...
        if "X4" in ix_:
            ...
            self.y0 = arrset(
                self.y0, np.where(self.congrps == ig_["X4"])[0], float(6.0)
            )
        if "X5" in ix_:
            self.x0[ix_["X5"]] = float(4.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X5"]), float(4.0)
            )
        # %%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_ = {}
        elftv = []
