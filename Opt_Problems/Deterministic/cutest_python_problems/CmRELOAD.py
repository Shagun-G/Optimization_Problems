    ...
    def __init__(self, *args):
        ...
        for I in range(int(v_["1"]), int(v_["N"]) + 1):
            for K in range(int(v_["1"]), int(v_["L"]) + 1):
                for J in range(int(v_["1"]), int(v_["M"]) + 1):
                    [iv, ix_, _] = s2mpj_ii(
                        "X" + str(I) + "," + str(K) + "," + str(J), ix_
                    )
                    self.xnames = arrset(
                        self.xnames, iv, "X" + str(I) + "," + str(K) + "," + str(J)
                    )
        ...
        for I in range(int(v_["1"]), int(v_["N"]) + 1):
            ...
            for J in range(int(v_["1"]), int(v_["M"]) + 1):
                ...
                irA = np.append(irA, [ig])
                icA = np.append(
                    icA, [ix_["X" + str(I) + "," + str(int(v_["1"])) + "," + str(J)]]
                )
        ...
        for K in range(int(v_["1"]), int(v_["L"]) + 1):
            for J in range(int(v_["1"]), int(v_["M"]) + 1):
                self.gconst = arrset(
                    self.gconst, ig_["SUMI" + str(K) + "," + str(J)], float(1.0)
                )
        for I in range(int(v_["1"]), int(v_["N"]) + 1):
        ...
        for I in range(int(v_["1"]), int(v_["N"]) + 1):
            for S in range(int(v_["1"]), int(v_["T"]) + 1):
                self.gconst = arrset(
                    self.gconst, ig_["PEAK" + str(I) + "," + str(S)], float(v_["TEMP"])
                )
        # %%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        for S in range(int(v_["1"]), int(v_["T"]) + 1):
            if "KEFF" + str(S) in ix_:
                ...
                self.y0 = arrset(
                    self.y0,
                    findfirst(self.congrps, lambda x: x == ig_["KEFF" + str(S)]),
                    float(v_["KEFFuINI"]),
                )
        for I in range(int(v_["1"]), int(v_["N"]) + 1):
            for S in range(int(v_["1"]), int(v_["T"]) + 1):
                if "KINF" + str(I) + "," + str(S) in ix_:
                    ...
                    self.y0 = arrset(
                        ...
                        findfirst(
                            self.congrps,
                            lambda x: x == ig_["KINF" + str(I) + "," + str(S)],
                        ),
                        float(v_["KFRESH"]),
                    )
                if "PHI" + str(I) + "," + str(S) in ix_:
                    ...
                    self.y0 = arrset(
                        ...
                        findfirst(
                            self.congrps,
                            lambda x: x == ig_["PHI" + str(I) + "," + str(S)],
                        ),
                        float(v_["TEMP"]),
                    )
        for I in range(int(v_["1"]), int(v_["N"]) + 1):
            for K in range(int(v_["1"]), int(v_["L"]) + 1):
                for J in range(int(v_["1"]), int(v_["M"]) + 1):
                    if "X" + str(I) + "," + str(K) + "," + str(J) in ix_:
                        self.x0[ix_["X" + str(I) + "," + str(K) + "," + str(J)]] = (
                            float(0.5)
                        )
                    else:
                        self.y0 = arrset(
        ...
        for S in range(int(v_["1"]), int(v_["T"]) + 1):
            for I in range(int(v_["1"]), int(v_["N"]) + 1):
                ...
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["KTP" + str(I) + "," + str(S)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                ...
                for II in range(int(v_["1"]), int(v_["NuROW"]) + 1):
                    ...
                    self.grelw = loaset(
                        ...
                        ig,
                        posel,
                        float(v_["G" + str(I) + "," + str(int(v_["III"]))]),
                    )
        for I in range(int(v_["1"]), int(v_["N"]) + 1):
            for S in range(int(v_["1"]), int(v_["T-1"]) + 1):
                ...
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["P" + str(I) + "," + str(S)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
