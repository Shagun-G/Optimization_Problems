...
class HS105(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        self.gconst = arrset(self.gconst, ig_["C1"], float(-1.0e0))
        # %%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        if "X1" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X1"]), float(0.1)
            )
        if "X2" in ix_:
            self.x0[ix_["X2"]] = float(0.2)
        else:
            self.y0 = arrset(
                self.y0, np.where(self.congrps == ig_["X2"])[0], float(0.2)
            )
        if "X3" in ix_:
            self.x0[ix_["X3"]] = float(100.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X3"]), float(100.0)
            )
        if "X4" in ix_:
            self.x0[ix_["X4"]] = float(125.0)
        else:
            self.y0 = arrset(
                self.y0, np.where(self.congrps == ig_["X4"])[0], float(125.0)
            )
        if "X5" in ix_:
            self.x0[ix_["X5"]] = float(175.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X5"]), float(175.0)
            )
        if "X6" in ix_:
            self.x0[ix_["X6"]] = float(11.2)
        else:
            self.y0 = arrset(
                self.y0, np.where(self.congrps == ig_["X6"])[0], float(11.2)
            )
        if "X7" in ix_:
            self.x0[ix_["X7"]] = float(13.2)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X7"]), float(13.2)
            )
        if "X8" in ix_:
            self.x0[ix_["X8"]] = float(15.8)
        else:
            self.y0 = arrset(
                self.y0, np.where(self.congrps == ig_["X8"])[0], float(15.8)
            )
        pass
        # %%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_ = {}
        ...
        if nargout > 1:
            ...
            if nargout > 2:
                ...
                H_[1, 1] = (
                    (
                        2.0e0 * D * DDV2 * E
                        + (D * D - 1.0e0) * (DEV2 - 2.0e0 * E / EV_[1])
                    )
                    * R
                    / EV_[1]
                )
                H_[1, 2] = (DDV2 * E + D * DEV2 - 2.0e0 * D * E / EV_[1]) * R / EV_[1]
                H_[2, 1] = H_[1, 2]
        ...
        if nargout > 1:
            ...
            if nargout > 2:
                ...
                H_[1, 1] = (
                    (
                        2.0e0 * D * DDV2 * E
                        + (D * D - 1.0e0) * (DEV2 - 2.0e0 * E / IV_[1])
                    )
                    * R
                    / IV_[1]
                )
                H_[1, 2] = (DDV2 * E + D * DEV2 - 2.0e0 * D * E / IV_[1]) * R / IV_[1]
                H_[2, 1] = H_[1, 2]
