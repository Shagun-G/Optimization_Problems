...
class RAT43(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        if "B1" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["B1"]), float(100.0)
            )
        if "B2" in ix_:
            self.x0[ix_["B2"]] = float(10.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["B2"]), float(10.0)
            )
        if "B3" in ix_:
            self.x0[ix_["B3"]] = float(1.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["B3"]), float(1.0)
            )
        if "B4" in ix_:
            self.x0[ix_["B4"]] = float(1.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["B4"]), float(1.0)
            )
        pass
        # %%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_ = {}
    ...
    def eE(self, nargout, *args):
        ...
        if nargout > 1:
            ...
            if nargout > 2:
                ...
                H_[3, 1] = H_[1, 3]
                H_[2, 2] = (
                    EV_[0] * self.elpar[iel_][0] ** 2 * (E2 * V4INVP / VE2 - E / VE)
                )
                H_[2, 3] = (
                    EV_[0] * self.elpar[iel_][0] * E * (EP1L / V42EP3 - 1.0 / V42EP2)
                )
                H_[3, 2] = H_[2, 3]
                H_[3, 3] = (EV_[0] / EP14) * (
                    EP1L**2 / EV_[3] ** 4 - 2.0 * EP1L / EV_[3] ** 3
                )
        if nargout == 1:
            return f_
        elif nargout == 2:
