...
class HS67(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        for I in range(int(v_["1"]), int(v_["7"]) + 1):
            v_["I+7"] = 7 + I
            self.gconst = arrset(
                self.gconst, ig_["AG" + str(I)], float(v_["A" + str(I)])
            )
            self.gconst = arrset(
                self.gconst, ig_["AL" + str(I)], float(v_["A" + str(int(v_["I+7"]))])
            )
        # %%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
    ...
    def extfunc(self, X1, X2, X3):
        ...
        for I in range(1, 1000):
            ...
            if np.abs(Y1C - Y[1]) > 0.001:
                ...
                Y[1] = Y1C
                G[1, 0] = (
                    0.01 * (112.0 + 13.167 * Y[5] - 0.6667 * Y[5] ** 2)
                    + X1 * 0.13167 * G[5, 0]
                    - X1 * 0.013334 * Y[5] * G[5, 0]
                )
                ...
                H[1, 1] = (
                    ...
                    - 0.013334 * Y[5] * G[5, 1]
                    + X1 * 0.13167 * H[5, 1]
                    - X1 * 0.013334 * G[5, 1] * G[5, 0]
                    - X1 * 0.013334 * Y[5] * H[5, 1]
                )
                H[1, 2] = (
                    0.13167 * G[5, 2]
                    - 0.013334 * Y[5] * G[5, 2]
                    + X1 * 0.13167 * H[5, 2]
                    - X1 * 0.013334 * G[5, 2] * G[5, 0]
                    - X1 * 0.013334 * Y[5] * H[5, 2]
                )
                H[1, 3] = X1 * (
                    0.13167 * H[5, 3]
                    - 0.013334 * G[5, 1] ** 2
        ...
        for I in range(1, 1000):
            ...
            if np.abs(Y3C - Y[3]) > 0.001:
                ...
                G[3, 2] = (
                    ...
                    - 98000.0
                    * X3
                    * (G[1, 2] * Y[6] + Y[1] * G[6, 2] + 1000.0)
                    / Y1Y6X3**2
                )
                H[3, 0] = (
                    -98000.0
                    * X3
                    * (H[1, 0] * Y[6] + 2.0 * G[1, 0] * G[6, 0] + Y[1] * H[6, 0])
                    / Y1Y6X3**2
                ...
                H[3, 2] = (
                    -98000.0 * (Y[1] * G[6, 0] + Y[6] * G[1, 0]) / Y1Y6X3**2
                    - 98000.0
                    * X3
                    * (
                        G[1, 2] * G[6, 0]
                ...
                H[3, 3] = (
                    -98000.0
                    * X3
                    * (H[1, 3] * Y[6] + 2.0 * G[1, 1] * G[6, 1] + Y[1] * H[6, 3])
                    / Y1Y6X3**2
                    + 196000.0 * X3 * (G[1, 1] * Y[6] + Y[1] * G[6, 1]) ** 2 / Y1Y6X3**3
                )
                H[3, 4] = (
                    -98000.0 * (Y[1] * G[6, 1] + Y[6] * G[1, 1]) / Y1Y6X3**2
                    - 98000.0
                    * X3
                    * (
                        G[1, 2] * G[6, 1]
                ...
                H[3, 5] = (
                    -196000.0 * (Y[1] * G[6, 2] + Y[6] * G[1, 2] + 1000.0) / Y1Y6X3**2
                    - 98000.0
                    * X3
                    * (H[1, 5] * Y[6] + 2.0 * G[1, 2] * G[6, 2] + Y[1] * H[6, 5])
                    / Y1Y6X3**2
                    + 196000.0
                    * X3
                    * (G[1, 2] * Y[6] + Y[1] * G[6, 2] + 1000.0) ** 2
                    / Y1Y6X3**3
                )
            else:
                break
    ...
    def eY2Y5(self, nargout, *args):
        ...
        if nargout > 1:
            ...
            if nargout > 2:
                H_ = np.zeros((3, 3))
                H_[0, 0] = (
                    YY[1] * HH[4, 0] + 2.0 * GG[4, 0] * GG[1, 0] + YY[4] * HH[1, 0]
                )
                H_[0, 1] = (
                    YY[1] * HH[4, 1]
                    + GG[4, 0] * GG[1, 1]
                ...
                H_[2, 0] = H_[0, 2]
                H_[1, 1] = (
                    YY[1] * HH[4, 3] + 2.0 * GG[4, 1] * GG[1, 1] + YY[4] * HH[1, 3]
                )
                H_[1, 2] = (
                    YY[1] * HH[4, 4]
                    + GG[4, 1] * GG[1, 2]
                ...
                H_[2, 1] = H_[1, 2]
                H_[2, 2] = (
                    YY[1] * HH[4, 5] + 2.0 * GG[4, 2] * GG[1, 2] + YY[4] * HH[1, 5]
                )
        if nargout == 1:
            return f_
        elif nargout == 2:
