...
class KOEBHELBNE(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        for I in range(int(v_["1"]), int(v_["M"]) + 1):
            self.gconst = arrset(
                self.gconst, ig_["O" + str(I)], float(v_["Y" + str(I)])
            )
        # %%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        if "N" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["N"]), float(10.0)
            )
        if "A" in ix_:
            self.x0[ix_["A"]] = float(0.01)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["A"]), float(0.01)
            )
        if "B" in ix_:
            self.x0[ix_["B"]] = float(0.01)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["B"]), float(0.01)
            )
        # %%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_ = {}
        elftv = []
