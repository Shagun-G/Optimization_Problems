...
class HS54(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        valA = np.append(valA, float(4.0e3))
        # %%%%%%%%%%%%%% GLOBAL DIMENSIONS %%%%%%%%%%%%%%%%%
        self.n = len(ix_)
        ngrp = len(ig_)
        ...
        v_["2000SI2"] = 2.0e3 * v_["SIGMA2"]
        v_["4000MU2"] = 4.0e3 * v_["MU2"]
        ...
        self.xupper[ix_["X1"]] = 2.0e4
        self.xlower[ix_["X2"]] = -1.0e1
        self.xupper[ix_["X2"]] = 1.0e1
        self.xupper[ix_["X3"]] = 1.0e7
        self.xupper[ix_["X4"]] = 2.0e1
        self.xlower[ix_["X5"]] = -1.0e0
        self.xupper[ix_["X5"]] = 1.0e0
        self.xupper[ix_["X6"]] = 2.0e8
        # %%%%%%%%%%%%%%%%%%% START POINT %%%%%%%%%%%%%%%%%%
        self.x0 = np.zeros((self.n, 1))
        self.y0 = np.zeros((self.m, 1))
        if "X1" in ix_:
            self.x0[ix_["X1"]] = float(6.0e3)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X1"]), float(6.0e3)
            )
        if "X2" in ix_:
            self.x0[ix_["X2"]] = float(1.5e0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X2"]), float(1.5e0)
            )
        if "X3" in ix_:
            self.x0[ix_["X3"]] = float(4.0e6)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X3"]), float(4.0e6)
            )
        if "X4" in ix_:
            self.x0[ix_["X4"]] = float(2.0e0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X4"]), float(2.0e0)
            )
        if "X5" in ix_:
        ...
        if "X6" in ix_:
            self.x0[ix_["X6"]] = float(5.0e7)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X6"]), float(5.0e7)
            )
        # %%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_ = {}
        elftv = []
