...
class EXPFITC(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        for I in range(int(v_["1"]), int(v_["R"]) + 1):
            self.gconst = arrset(
                self.gconst, ig_["C" + str(I)], float(v_["ET" + str(I)])
            )
        ...
        if "P0" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["P0"]), float(1.0)
            )
        if "P1" in ix_:
            self.x0[ix_["P1"]] = float(1.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["P1"]), float(1.0)
            )
        if "P2" in ix_:
            self.x0[ix_["P2"]] = float(6.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["P2"]), float(6.0)
            )
        if "Q1" in ix_:
            self.x0[ix_["Q1"]] = float(0.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["Q1"]), float(0.0)
            )
        if "Q2" in ix_:
            self.x0[ix_["Q2"]] = float(0.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["Q2"]), float(0.0)
            )
        # %%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_ = {}
        elftv = []
