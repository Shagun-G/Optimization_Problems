    ...
    def __init__(self, *args):
        ...
        for I in range(int(v_["0"]), int(v_["NX"]) + 1):
            for J in range(int(v_["0"]), int(v_["NY"]) + 1):
                ...
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["I" + str(I) + "," + str(J)]
                )
                self.grelw = loaset(self.grelw, ig, posel, float(v_["1/HX2"]))
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["J" + str(I) + "," + str(J)]
                )
        ...
        for I in range(int(v_["1"]), int(v_["NX+1"]) + 1):
            ...
            for J in range(int(v_["1"]), int(v_["NY+1"]) + 1):
                ...
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["I" + str(int(v_["I-1"])) + "," + str(J)]
                )
                self.grelw = loaset(self.grelw, ig, posel, float(v_["1/HX2"]))
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["J" + str(I) + "," + str(int(v_["J-1"]))]
                )
                self.grelw = loaset(self.grelw, ig, posel, float(v_["1/HY2"]))
        # %%%%%%%%%%%%%%%%%% OBJECT BOUNDS %%%%%%%%%%%%%%%%%
        #    Solution
