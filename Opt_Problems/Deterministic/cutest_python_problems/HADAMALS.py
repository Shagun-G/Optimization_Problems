...
class HADAMALS(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        for J in range(int(v_["1"]), int(v_["N"]) + 1):
            self.gconst = arrset(
                self.gconst, ig_["O" + str(J) + "," + str(J)], float(v_["RN"])
            )
        for J in range(int(v_["1"]), int(v_["N"]) + 1):
            for I in range(int(v_["2"]), int(v_["N"]) + 1):
                self.gconst = arrset(
                    self.gconst, ig_["S" + str(I) + "," + str(J)], float(1.0)
                )
        # %%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        for J in range(int(v_["1"]), int(v_["N"]) + 1):
            for I in range(int(v_["1"]), int(J) + 1):
                for K in range(int(v_["1"]), int(v_["N"]) + 1):
                    ...
                    vname = "Q" + str(K) + "," + str(I)
                    [iv, ix_] = s2mpj_nlx(
                        self, vname, ix_, 1, float(-1.0), float(1.0), None
                    )
    ...
    def g_globs(self):
        ...
        self.gfpar = np.array([])
        self.gfpar = arrset(self.gfpar, 0, 1.0e0)  # this is FACTOR
        return pbm
    ...
    def gLARGEL2(self, nargout, *args):
        ...
        if nargout > 1:
            g_ = 2.0e0 * self.gfpar[0] * GVAR_
            if nargout > 2:
                H_ = np.zeros((1, 1))
                H_ = 2.0e0 * self.gfpar[0]
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_, g_
