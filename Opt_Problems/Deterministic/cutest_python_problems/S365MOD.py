...
class S365MOD(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        if "X1" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X1"]), float(3.0)
            )
        if "X2" in ix_:
            self.x0[ix_["X2"]] = float(0.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X2"]), float(0.0)
            )
        if "X3" in ix_:
            self.x0[ix_["X3"]] = float(2.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X3"]), float(2.0)
            )
        if "X4" in ix_:
            self.x0[ix_["X4"]] = float(-1.5)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X4"]), float(-1.5)
            )
        if "X5" in ix_:
            self.x0[ix_["X5"]] = float(1.5)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X5"]), float(1.5)
            )
        if "X6" in ix_:
            self.x0[ix_["X6"]] = float(5.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X6"]), float(5.0)
            )
        if "X7" in ix_:
            self.x0[ix_["X7"]] = float(0.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X7"]), float(0.0)
            )
        # %%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_ = {}
        elftv = []
