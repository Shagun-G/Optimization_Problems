...
class GULFNE(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        if "X1" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X1"]), float(5.0)
            )
        if "X2" in ix_:
            self.x0[ix_["X2"]] = float(2.5)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X2"]), float(2.5)
            )
        if "X3" in ix_:
            self.x0[ix_["X3"]] = float(0.15)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X3"]), float(0.15)
            )
        # %%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_ = {}
        elftv = []
