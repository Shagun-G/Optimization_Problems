...
class HS114(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        if "X3" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X3"]), float(110.0)
            )
        if "X4" in ix_:
        ...
        if "X6" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X6"]), float(89.2)
            )
        if "X7" in ix_:
            self.x0[ix_["X7"]] = float(92.8)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X7"]), float(92.8)
            )
        if "X8" in ix_:
        ...
        if "X9" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X9"]), float(3.6)
            )
        if "X10" in ix_:
