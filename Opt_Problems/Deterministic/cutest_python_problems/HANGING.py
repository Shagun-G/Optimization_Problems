...
class HANGING(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        for I in range(int(v_["1"]), int(v_["NX"]) + 1):
            for J in range(int(v_["1"]), int(v_["NY-1"]) + 1):
                self.gconst = arrset(
                    self.gconst, ig_["RC" + str(I) + "," + str(J)], float(v_["LX2"])
                )
        for I in range(int(v_["1"]), int(v_["NX-1"]) + 1):
            for J in range(int(v_["1"]), int(v_["NY"]) + 1):
                self.gconst = arrset(
                    self.gconst, ig_["DC" + str(I) + "," + str(J)], float(v_["LY2"])
                )
        # %%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        self.xlower[ix_["X" + str(int(v_["NX"])) + "," + str(int(v_["NY"]))]] = v_[
            "RNX"
        ]
        self.xupper[ix_["X" + str(int(v_["NX"])) + "," + str(int(v_["NY"]))]] = v_[
            "RNX"
        ]
        self.xlower[ix_["Y" + str(int(v_["NX"])) + "," + str(int(v_["NY"]))]] = v_[
            "RNY"
        ]
        self.xupper[ix_["Y" + str(int(v_["NX"])) + "," + str(int(v_["NY"]))]] = v_[
            "RNY"
        ]
        ...
        for I in range(int(v_["1"]), int(v_["NX"]) + 1):
            for J in range(int(v_["1"]), int(v_["NY-1"]) + 1):
                ...
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["RX" + str(I) + "," + str(J)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, 1.0)
                posel = posel + 1
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["RY" + str(I) + "," + str(J)]
                )
                self.grelw = loaset(self.grelw, ig, posel, 1.0)
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["RZ" + str(I) + "," + str(J)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, 1.0)
        for I in range(int(v_["1"]), int(v_["NX-1"]) + 1):
            for J in range(int(v_["1"]), int(v_["NY"]) + 1):
                ...
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["DX" + str(I) + "," + str(J)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, 1.0)
                posel = posel + 1
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["DY" + str(I) + "," + str(J)]
                )
                self.grelw = loaset(self.grelw, ig, posel, 1.0)
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["DZ" + str(I) + "," + str(J)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, 1.0)
        # %%%%%%%%%%%%%%%%%% OBJECT BOUNDS %%%%%%%%%%%%%%%%%
