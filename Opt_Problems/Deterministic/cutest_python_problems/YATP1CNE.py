    ...
    def __init__(self, *args):
        ...
        for I in range(int(v_["1"]), int(v_["N"]) + 1):
            for J in range(int(v_["1"]), int(v_["N"]) + 1):
                ...
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["CB" + str(I) + "," + str(J)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, float(1.0))
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["SQ" + str(I) + "," + str(J)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, float(v_["-A"]))
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["DC" + str(I) + "," + str(J)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, float(-1.0))
                posel = posel + 1
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["DS" + str(I) + "," + str(J)]
                )
                self.grelw = loaset(self.grelw, ig, posel, float(1.0))
        for I in range(int(v_["1"]), int(v_["N"]) + 1):
            for J in range(int(v_["1"]), int(v_["N"]) + 1):
                ...
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["SX" + str(I) + "," + str(J)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
