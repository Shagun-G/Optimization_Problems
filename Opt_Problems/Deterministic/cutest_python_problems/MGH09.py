...
class MGH09(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        if "B1" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["B1"]), float(25.0)
            )
        if "B2" in ix_:
            self.x0[ix_["B2"]] = float(39.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["B2"]), float(39.0)
            )
        if "B3" in ix_:
            self.x0[ix_["B3"]] = float(41.5)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["B3"]), float(41.5)
            )
        if "B4" in ix_:
            self.x0[ix_["B4"]] = float(39.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["B4"]), float(39.0)
            )
        pass
        # %%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_ = {}
