...
class FEEDLOC(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        for I in range(int(v_["2"]), int(v_["NMAX"]) + 1):
            for J in range(int(v_["1"]), int(v_["M"]) + 1):
                self.gconst = arrset(
                    self.gconst, ig_["CMBN1" + str(I) + "," + str(J)], float(v_["BIGM"])
                )
                self.gconst = arrset(
                    self.gconst, ig_["CMBN2" + str(I) + "," + str(J)], float(v_["TEMP"])
                )
        for I in range(int(v_["2"]), int(v_["NMAX-1"]) + 1):
        ...
        for I in range(int(v_["1"]), int(v_["NMAX"]) + 1):
            for K in range(int(v_["1"]), int(v_["M"]) + 1):
                ...
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["PHE" + str(I) + "," + str(K)]
                )
        ...
        for I in range(int(v_["2"]), int(v_["NMAX"]) + 1):
            for J in range(int(v_["1"]), int(v_["M"]) + 1):
                ...
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["CM11" + str(I) + "," + str(J)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, 1.0)
                posel = posel + 1
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["CM12" + str(I) + "," + str(J)]
                )
                self.grelw = loaset(self.grelw, ig, posel, 1.0)
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["CM13" + str(I) + "," + str(J)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, 1.0)
                ...
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["CM21" + str(I) + "," + str(J)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, 1.0)
                posel = posel + 1
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["CM22" + str(I) + "," + str(J)]
                )
                self.grelw = loaset(self.grelw, ig, posel, 1.0)
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["CM23" + str(I) + "," + str(J)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, 1.0)
        for I in range(int(v_["2"]), int(v_["NMAX-1"]) + 1):
            ...
            for K in range(int(I), int(v_["NMAX"]) + 1):
                ...
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["C12" + str(I) + "," + str(K)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, 1.0)
                ig = ig_["CMB2" + str(I)]
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["C22" + str(I) + "," + str(K)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, 1.0)
            v_["I+1"] = 1 + I
            for K in range(int(v_["I+1"]), int(v_["NMAX"]) + 1):
                ...
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["C15" + str(I) + "," + str(K)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, 1.0)
                ig = ig_["CMB2" + str(I)]
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["C25" + str(I) + "," + str(K)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, 1.0)
        for I in range(int(v_["3"]), int(v_["NMAX"]) + 1):
