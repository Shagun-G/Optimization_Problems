...
class HS68(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        self.elpar = loaset(self.elpar, ie, posep[0], float(0.0e0))
        ename = "C2E1"
        [ie, ie_, _] = s2mpj_ii(ename, ie_)
        self.elftype = arrset(self.elftype, ie, "ePHI")
        ielftype = arrset(ielftype, ie, iet_["ePHI"])
    ...
    def eNASTYEXP(self, nargout, *args):
        ...
        iel_ = args[1]
        E = np.exp(EV_[0])
        R = self.elpar[iel_][0] * (E - 1.0e0) - EV_[1]
        S = E - 1.0e0 + EV_[2]
        F1 = -(-(R * EV_[2] / S)) / (EV_[0] * EV_[0])
        F2 = -R / (S * EV_[0])
        F3 = -EV_[2] / (S * EV_[0])
        F4 = (EV_[2] * R) / (EV_[0] * S * S)
        F11 = 2.0e0 * (-(R * EV_[2] / S)) / (EV_[0] * EV_[0] * EV_[0])
        ...
        if nargout > 1:
            ...
            if nargout > 2:
                ...
                H_[0, 0] = (
                    -F11
                    - (2.0e0 * self.elpar[iel_][0] * E * F13)
                    - (2.0e0 * E * F16)
                    - (2.0e0 * self.elpar[iel_][0] * E * E * F17)
                    - (E * E * F18)
                )
                H_[1, 0] = F13 + (E * F17)
                H_[0, 1] = H_[1, 0]
                H_[2, 0] = (
                    -(E * F14)
                    - (self.elpar[iel_][0] * E * F15)
                    - F16
                    - (self.elpar[iel_][0] * E * F17)
                    - (E * F18)
                    - F12
                )
                H_[0, 2] = H_[2, 0]
