...
class YORKNET(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        for J in range(int(v_["SuRES"]), int(v_["EuRES"]) + 1):
            self.gconst = arrset(
                self.gconst,
                ig_["RD" + str(J) + "," + str(int(v_["1"]))],
                float(v_["MSTVOL" + str(J)]),
            )
        # %%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        for I in range(int(v_["1"]), int(v_["NSTEP"]) + 1):
            for J in range(int(v_["SuPIPE"]), int(v_["EuPIPE"]) + 1):
                ...
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["EA" + str(J) + "," + str(I)]
                )
            ...
            for J in range(int(v_["SuPMP"]), int(v_["EuPMP"]) + 1):
                ...
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["EB" + str(J) + "," + str(I)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, float(v_["CONB" + str(J)]))
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["EC" + str(J) + "," + str(I)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, float(v_["CONC" + str(J)]))
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["EH" + str(J) + "," + str(I)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
        ...
        for J in range(int(v_["SuPMP"]), int(v_["EuPMP"]) + 1):
            ...
            posel = len(self.grelt[ig])
            self.grelt = loaset(
                self.grelt, ig, posel, ie_["PPW" + str(J) + "," + str(int(v_["2"]))]
            )
            nlc = np.union1d(nlc, np.array([ig]))
            ...
            posel = len(self.grelt[ig])
            self.grelt = loaset(
                self.grelt, ig, posel, ie_["PPW" + str(J) + "," + str(int(v_["3"]))]
            )
            nlc = np.union1d(nlc, np.array([ig]))
            self.grelw = loaset(self.grelw, ig, posel, float(100.0))
            ...
            posel = len(self.grelt[ig])
            self.grelt = loaset(
                self.grelt, ig, posel, ie_["PPW" + str(J) + "," + str(int(v_["4"]))]
            )
            nlc = np.union1d(nlc, np.array([ig]))
            self.grelw = loaset(self.grelw, ig, posel, float(100.0))
