...
class OSBORNE1(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        if "X2" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X2"]), float(1.5)
            )
        if "X3" in ix_:
            self.x0[ix_["X3"]] = float(-1.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X3"]), float(-1.0)
            )
        if "X4" in ix_:
            self.x0[ix_["X4"]] = float(0.01)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X4"]), float(0.01)
            )
        if "X5" in ix_:
            self.x0[ix_["X5"]] = float(0.02)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X5"]), float(0.02)
            )
        # %%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_ = {}
        elftv = []
