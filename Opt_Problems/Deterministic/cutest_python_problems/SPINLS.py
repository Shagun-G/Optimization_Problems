    ...
    def __init__(self, *args):
        ...
        for I in range(int(v_["1"]), int(v_["N"]) + 1):
            ...
            for J in range(int(v_["I+1"]), int(v_["N"]) + 1):
                ...
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["RY" + str(J) + "," + str(I)]
                )
                self.grelw = loaset(self.grelw, ig, posel, float(-1.0))
            ig = ig_["I" + str(I)]
            posel = len(self.grelt[ig])
            ...
            for J in range(int(v_["I+1"]), int(v_["N"]) + 1):
                ...
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["RX" + str(J) + "," + str(I)]
                )
                self.grelw = loaset(self.grelw, ig, posel, float(1.0))
        for I in range(int(v_["2"]), int(v_["N"]) + 1):
            v_["I-1"] = -1 + I
            for J in range(int(v_["1"]), int(v_["I-1"]) + 1):
                ...
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["V" + str(I) + "," + str(J)]
                )
                self.grelw = loaset(self.grelw, ig, posel, float(-1.0))
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["X" + str(I) + "," + str(J)]
                )
                self.grelw = loaset(self.grelw, ig, posel, float(1.0))
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["Y" + str(I) + "," + str(J)]
                )
                self.grelw = loaset(self.grelw, ig, posel, float(1.0))
