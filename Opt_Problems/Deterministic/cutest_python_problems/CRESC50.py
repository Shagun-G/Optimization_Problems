...
class CRESC50(CUTEst_problem):
    ...
    def eSC(self, nargout, *args):
        ...
        RAD = (
            2.0 * (BD * BA * C + B * BAD * C + B * BA * CD + B * BD * CA) + B * B * CAD
        )
        ...
        WAB = 0.5 * (
            EAB * SQ + EA * CQ * QB + EB * CQ * QA - E * SQ * QB * QA + E * CQ * QAB
        )
        WAD = 0.5 * (
            EAD * SQ + EA * CQ * QD + ED * CQ * QA - E * SQ * QD * QA + E * CQ * QAD
        )
        WBB = 0.5 * (EB * CQ * QB + EB * CQ * QB - E * SQ * QB * QB + E * CQ * QBB)
        WBD = 0.5 * (
            EBD * SQ + EB * CQ * QD + ED * CQ * QB - E * SQ * QD * QB + E * CQ * QBD
        )
        WDD = 0.5 * (
            EDD * SQ + ED * CQ * QD + ED * CQ * QD - E * SQ * QD * QD + E * CQ * QDD
        )
        V = S - R + W
        VA = SA - RA + WA
        VB = SB - RB + WB
