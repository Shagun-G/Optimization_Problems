...
class OPTPRLOC(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        for I in range(int(v_["1"]), int(v_["N"]) + 1):
            self.gconst = arrset(
                self.gconst, ig_["ELLI" + str(I)], float(v_["RPH" + str(I)])
            )
        ...
        for I in range(int(v_["1"]), int(v_["N"]) + 1):
            for L in range(int(v_["1"]), int(v_["K"]) + 1):
                ...
                posep = np.where(elftp[ielftype[ie]] == "B")[0]
                self.elpar = loaset(
                    self.elpar, ie, posep[0], float(v_["Z" + str(I) + "," + str(L)])
                )
        # %%%%%%%%%%%%%%%%%%% GROUP USES %%%%%%%%%%%%%%%%%%%
        self.grelt = []
        ...
        for I in range(int(v_["1"]), int(v_["N"]) + 1):
            for L in range(int(v_["1"]), int(v_["K"]) + 1):
                ...
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["SQ" + str(I) + "," + str(L)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(
                    self.grelw, ig, posel, float(v_["W" + str(I) + "," + str(L)])
                )
        # %%%%%%%%%%%%%%%%%% OBJECT BOUNDS %%%%%%%%%%%%%%%%%
        #    Solution
