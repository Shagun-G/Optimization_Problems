...
class RK23(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        if "C2" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["C2"]), float(1.0)
            )
        if "A21" in ix_:
            self.x0[ix_["A21"]] = float(1.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["A21"]), float(1.0)
            )
        if "C3" in ix_:
            self.x0[ix_["C3"]] = float(0.5)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["C3"]), float(0.5)
            )
        if "A31" in ix_:
            self.x0[ix_["A31"]] = float(0.25)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["A31"]), float(0.25)
            )
        if "A32" in ix_:
            self.x0[ix_["A32"]] = float(0.25)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["A32"]), float(0.25)
            )
        if "B1" in ix_:
            self.x0[ix_["B1"]] = float(0.5)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["B1"]), float(0.5)
            )
        if "B2" in ix_:
            self.x0[ix_["B2"]] = float(0.5)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["B2"]), float(0.5)
            )
        if "B3" in ix_:
            self.x0[ix_["B3"]] = float(0.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["B3"]), float(0.0)
            )
