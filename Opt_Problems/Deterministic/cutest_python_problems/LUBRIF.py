...
class LUBRIF(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        for J in range(int(v_["1"]), int(v_["2N-1"]) + 1, int(v_["2"])):
            ...
            v_["COEFF"] = v_["T1"] * v_["1/2PI"]
            v_["-COEFF"] = -1.0 * v_["COEFF"]
            v_["C" + str(int(v_["2N-2"]))] = (
                v_["C" + str(int(v_["2N-2"]))] + v_["-COEFF"]
            )
            for I in range(int(v_["2"]), int(v_["2N-2"]) + 1, int(v_["2"])):
                [ig, ig_, _] = s2mpj_ii("DH" + str(J), ig_)
                gtype = arrset(gtype, ig, "==")
        ...
        for I in range(int(v_["2"]), int(v_["4NN"]) + 1, int(v_["2"])):
            ...
            if "P" + str(I) in ix_:
                ...
                self.y0 = arrset(
                    self.y0,
                    findfirst(self.congrps, lambda x: x == ig_["P" + str(I)]),
                    float(v_["PI0"]),
                )
        v_["4NN+2"] = 2 + v_["4NN"]
        v_["8NN"] = 8 * v_["NN"]
        ...
        for J in range(int(v_["1"]), int(v_["2N-1"]) + 1, int(v_["2"])):
            ...
            if "H" + str(J) in ix_:
                ...
                self.y0 = arrset(
                    self.y0,
                    findfirst(self.congrps, lambda x: x == ig_["H" + str(J)]),
                    float(v_["XJSQ"]),
                )
        # %%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_ = {}
