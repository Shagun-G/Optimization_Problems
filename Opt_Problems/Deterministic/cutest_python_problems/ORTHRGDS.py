...
class ORTHRGDS(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        if "Z1" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["Z1"]), float(1.0)
            )
        if "Z2" in ix_:
            self.x0[ix_["Z2"]] = float(0.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["Z2"]), float(0.0)
            )
        if "Z3" in ix_:
            self.x0[ix_["Z3"]] = float(1.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["Z3"]), float(1.0)
            )
        for I in range(int(v_["1"]), int(v_["NPTS"]) + 1):
            if "X" + str(I) in ix_:
