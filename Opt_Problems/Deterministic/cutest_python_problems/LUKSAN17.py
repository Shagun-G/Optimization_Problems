...
class LUKSAN17(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        for J in range(int(v_["1"]), int(v_["S"]) + 1):
            self.gconst = arrset(
                self.gconst, ig_["E" + str(int(v_["K"]))], float(v_["Y1"])
            )
            v_["K"] = 1 + v_["K"]
            self.gconst = arrset(
                self.gconst, ig_["E" + str(int(v_["K"]))], float(v_["Y2"])
            )
            v_["K"] = 1 + v_["K"]
            self.gconst = arrset(
                self.gconst, ig_["E" + str(int(v_["K"]))], float(v_["Y3"])
            )
            v_["K"] = 1 + v_["K"]
            self.gconst = arrset(
                self.gconst, ig_["E" + str(int(v_["K"]))], float(v_["Y4"])
            )
            v_["K"] = 1 + v_["K"]
        # %%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        for K in range(int(v_["1"]), int(v_["M"]) + 1):
            for Q in range(int(v_["1"]), int(v_["4"]) + 1):
                ...
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["S" + str(K) + "," + str(Q)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, 1.0)
                posel = posel + 1
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["C" + str(K) + "," + str(Q)]
                )
                self.grelw = loaset(self.grelw, ig, posel, 1.0)
        # %%%%%%%%%%%%%%%%%% OBJECT BOUNDS %%%%%%%%%%%%%%%%%
        self.objlower = 0.0
