...
class DIPIGRI(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        if "X3" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X3"]), float(0.0)
            )
        if "X4" in ix_:
        ...
        if "X5" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X5"]), float(0.0)
            )
        if "X6" in ix_:
            self.x0[ix_["X6"]] = float(1.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X6"]), float(1.0)
            )
        if "X7" in ix_:
