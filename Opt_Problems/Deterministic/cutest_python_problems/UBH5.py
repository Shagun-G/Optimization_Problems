    ...
    def __init__(self, *args):
        ...
        for I in range(int(v_["1"]), int(v_["3"]) + 1):
            ...
            for T in range(int(v_["1"]), int(v_["N"]) + 1):
                ...
                irA = np.append(irA, [ig])
                icA = np.append(
                    icA, [ix_["Y" + str(int(v_["I+3"])) + "," + str(int(v_["T-1"]))]]
                )
        ...
        for T in range(int(v_["1"]), int(v_["N"]) + 1):
            ...
            irA = np.append(irA, [ig])
            icA = np.append(
                icA, [ix_["Y" + str(int(v_["7"])) + "," + str(int(v_["T-1"]))]]
            )
            valA = np.append(valA, float(-1.0))
        # %%%%%%%%%%%%%% GLOBAL DIMENSIONS %%%%%%%%%%%%%%%%%
        self.n = len(ix_)
        ...
        for T in range(int(v_["1"]), int(v_["N"]) + 1):
            ...
            for I in range(int(v_["1"]), int(v_["3"]) + 1):
                ...
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["E" + str(I) + "," + str(int(v_["T-1"]))]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, float(v_["-K/2"]))
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["E" + str(I) + "," + str(T)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, float(v_["-K/2"]))
        # %%%%%%%%%%%%%%%%%% OBJECT BOUNDS %%%%%%%%%%%%%%%%%
