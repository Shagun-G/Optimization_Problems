...
class ORTHREGF(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        for I in range(int(v_["1"]), int(v_["NPTS"]) + 1):
            for J in range(int(v_["1"]), int(v_["NPTS"]) + 1):
                self.gconst = arrset(
                    self.gconst,
                    ig_["OX" + str(I) + "," + str(J)],
                    float(v_["XD" + str(I) + "," + str(J)]),
                )
                self.gconst = arrset(
                    self.gconst,
                    ig_["OY" + str(I) + "," + str(J)],
                    float(v_["YD" + str(I) + "," + str(J)]),
                )
                self.gconst = arrset(
                    self.gconst,
                    ig_["OZ" + str(I) + "," + str(J)],
                    float(v_["ZD" + str(I) + "," + str(J)]),
                )
        # %%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        if "P2" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["P2"]), float(0.0)
            )
        if "P3" in ix_:
            self.x0[ix_["P3"]] = float(1.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["P3"]), float(1.0)
            )
        if "P4" in ix_:
            self.x0[ix_["P4"]] = float(1.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["P4"]), float(1.0)
            )
        if "P5" in ix_:
            self.x0[ix_["P5"]] = float(0.5)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["P5"]), float(0.5)
            )
        for I in range(int(v_["1"]), int(v_["NPTS"]) + 1):
            for J in range(int(v_["1"]), int(v_["NPTS"]) + 1):
                if "X" + str(I) + "," + str(J) in ix_:
                    self.x0[ix_["X" + str(I) + "," + str(J)]] = float(
                        v_["XD" + str(I) + "," + str(J)]
                    )
                else:
                    self.y0 = arrset(
                ...
                if "Y" + str(I) + "," + str(J) in ix_:
                    self.x0[ix_["Y" + str(I) + "," + str(J)]] = float(
                        v_["YD" + str(I) + "," + str(J)]
                    )
                else:
                    self.y0 = arrset(
                        ...
                        findfirst(
                            self.congrps,
                            lambda x: x == ig_["Y" + str(I) + "," + str(J)],
                        ),
                        float(v_["YD" + str(I) + "," + str(J)]),
                    )
                if "Z" + str(I) + "," + str(J) in ix_:
                    self.x0[ix_["Z" + str(I) + "," + str(J)]] = float(
                        v_["ZD" + str(I) + "," + str(J)]
                    )
                else:
                    self.y0 = arrset(
        ...
        for I in range(int(v_["1"]), int(v_["NPTS"]) + 1):
            for J in range(int(v_["1"]), int(v_["NPTS"]) + 1):
                ...
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["EA" + str(I) + "," + str(J)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, 1.0)
                posel = posel + 1
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["EB" + str(I) + "," + str(J)]
                )
                self.grelw = loaset(self.grelw, ig, posel, 1.0)
                posel = len(self.grelt[ig])
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["EC" + str(I) + "," + str(J)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
