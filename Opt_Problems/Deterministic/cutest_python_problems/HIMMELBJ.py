    ...
    def __init__(self, *args):
        ...
        for K in range(int(v_["1"]), int(v_["NSETS"]) + 1):
            ...
            for J in range(int(v_["1"]), int(v_["NSK"]) + 1):
                ...
                vname = "X" + str(J) + "," + str(K)
                [iv, ix_] = s2mpj_nlx(
                    self, vname, ix_, 1, float(1.0e-12), None, float(0.1)
                )
        ...
        for J in range(int(v_["1"]), int(v_["4"]) + 1):
            ...
            [iv, ix_] = s2mpj_nlx(self, vname, ix_, 1, float(1.0e-12), None, float(0.1))
            posev = np.where(elftv[ielftype[ie]] == "X")[0]
            self.elvar = loaset(self.elvar, ie, posev[0], iv)
            ename = "B" + str(J) + "," + str(int(v_["K"]))
            [ie, ie_, _] = s2mpj_ii(ename, ie_)
        ...
        for K in range(int(v_["1"]), int(v_["NSETS"]) + 1):
            ...
            for J in range(int(v_["1"]), int(v_["NSK"]) + 1):
                ...
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["A" + str(J) + "," + str(K)]
                )
                nlc = np.union1d(nlc, np.array([ig]))
                self.grelw = loaset(self.grelw, ig, posel, 1.0)
                posel = posel + 1
                self.grelt = loaset(
                    self.grelt, ig, posel, ie_["B" + str(J) + "," + str(K)]
                )
