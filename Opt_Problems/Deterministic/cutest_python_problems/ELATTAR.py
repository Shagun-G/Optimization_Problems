...
class ELATTAR(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        for I in range(int(v_["1"]), int(v_["51"]) + 1):
            ...
            self.gconst = arrset(
                self.gconst, ig_["MF" + str(I)], float(v_["-Y" + str(I)])
            )
        # %%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        if "X3" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X3"]), float(7.0)
            )
        if "X5" in ix_:
            self.x0[ix_["X5"]] = float(-2.0)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X5"]), float(-2.0)
            )
        if "X6" in ix_:
