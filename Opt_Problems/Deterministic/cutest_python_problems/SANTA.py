...
class SANTA(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        for I in range(int(v_["4"]), int(v_["S-1"]) + 1):
            ...
            [iv, ix_] = s2mpj_nlx(
                self, vname, ix_, 1, float(-1000.0), float(1000.0), None
            )
            posev = np.where(elftv[ielftype[ie]] == "PHI1")[0]
            self.elvar = loaset(self.elvar, ie, posev[0], iv)
            ename = "E" + str(int(v_["I2"])) + "," + str(I)
        ...
        for I in range(int(v_["2"]), int(v_["S"]) + 1):
            ...
            posel = len(self.grelt[ig])
            self.grelt = loaset(
                self.grelt, ig, posel, ie_["E" + str(int(v_["I2"])) + "," + str(I)]
            )
            nlc = np.union1d(nlc, np.array([ig]))
            self.grelw = loaset(self.grelw, ig, posel, 1.0)
            v_["I1"] = -1 + I
            ...
            posel = len(self.grelt[ig])
            self.grelt = loaset(
                self.grelt, ig, posel, ie_["E" + str(int(v_["I1"])) + "," + str(I)]
            )
            nlc = np.union1d(nlc, np.array([ig]))
            self.grelw = loaset(self.grelw, ig, posel, 1.0)
        # %%%%%%%%%%%%%%%%%% OBJECT BOUNDS %%%%%%%%%%%%%%%%%
