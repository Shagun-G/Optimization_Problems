...
class HAGER4(CUTEst_problem):
    ...
    def eELT(self, nargout, *args):
        ...
        iel_ = args[1]
        f_ = (
            self.elpar[iel_][0] * EV_[0] * EV_[0]
            + self.elpar[iel_][1] * EV_[0] * (EV_[1] - EV_[0])
            + self.elpar[iel_][2] * (EV_[1] - EV_[0]) ** 2
        )
        ...
        if nargout > 1:
            ...
            g_ = np.zeros(dim)
            g_[0] = (
                2.0 * self.elpar[iel_][0] * EV_[0]
                + self.elpar[iel_][1] * (EV_[1] - 2.0 * EV_[0])
                - 2.0 * self.elpar[iel_][2] * (EV_[1] - EV_[0])
            )
            g_[1] = self.elpar[iel_][1] * EV_[0] + 2.0 * self.elpar[iel_][2] * (
                EV_[1] - EV_[0]
            )
            if nargout > 2:
                H_ = np.zeros((2, 2))
                H_[0, 0] = 2.0 * (
                    self.elpar[iel_][0] - self.elpar[iel_][1] + self.elpar[iel_][2]
                )
