...
class DEVGLA2NE(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        if "X1" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X1"]), float(20.0)
            )
        if "X2" in ix_:
        ...
        if "X4" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X4"]), float(2.0)
            )
        if "X5" in ix_:
            self.x0[ix_["X5"]] = float(0.2)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X5"]), float(0.2)
            )
        pass
        # %%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_ = {}
    ...
    def eDG2(self, nargout, *args):
        ...
        F2X2X2 = (
            self.elpar[iel_][0]
            * (self.elpar[iel_][0] - 1.0e0)
            * EV_[1] ** (self.elpar[iel_][0] - 2.0e0)
        )
