...
class ACOPP14(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        # %%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        self.xlower = np.full((self.n, 1), -1.0e30)
        self.xupper = np.full((self.n, 1), 1.0e30)
        ...
        vname = "M1"
        [iv, ix_] = s2mpj_nlx(
            self, vname, ix_, 1, float(-1.0e30), float(1.0e30), float(0.0)
        )
    ...
    def eP31(self, nargout, *args):
        ...
        if nargout > 1:
            try:
                ...
                dim = len(EV_)
            g_ = np.zeros(dim)
            g_[0] = 3.0e0 * EV_[1] * EV_[0] ** 2
            g_[1] = EV_[0] ** 3
            if nargout > 2:
                H_ = np.zeros((2, 2))
                H_[0, 0] = 6.0e0 * EV_[1] * EV_[0]
                H_[0, 1] = 3.0e0 * EV_[0] ** 2
                H_[1, 0] = H_[0, 1]
        if nargout == 1:
            return f_
        elif nargout == 2:
    ...
    def eP22(self, nargout, *args):
        ...
        if nargout > 1:
            try:
                ...
                dim = len(EV_)
            g_ = np.zeros(dim)
            g_[0] = 2.0e0 * EV_[0] * EV_[1] ** 2
            g_[1] = 2.0e0 * EV_[1] * EV_[0] ** 2
            if nargout > 2:
                H_ = np.zeros((2, 2))
                H_[0, 0] = 2.0e0 * EV_[1] ** 2
                H_[0, 1] = 4.0e0 * EV_[0] * EV_[1]
                H_[1, 0] = H_[0, 1]
                H_[1, 1] = 2.0e0 * EV_[0] ** 2
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_, g_
    ...
    def eP211(self, nargout, *args):
        ...
        if nargout > 1:
            try:
                ...
                dim = len(EV_)
            g_ = np.zeros(dim)
            g_[0] = 2.0e0 * EV_[2] * EV_[1] * EV_[0]
            g_[1] = EV_[2] * EV_[0] ** 2
            g_[2] = EV_[1] * EV_[0] ** 2
            if nargout > 2:
                H_ = np.zeros((3, 3))
                H_[0, 0] = 2.0e0 * EV_[2] * EV_[1]
                H_[0, 1] = 2.0e0 * EV_[2] * EV_[0]
                H_[1, 0] = H_[0, 1]
                H_[0, 2] = 2.0e0 * EV_[1] * EV_[0]
                H_[2, 0] = H_[0, 2]
                H_[1, 2] = EV_[0] ** 2
                H_[2, 1] = H_[1, 2]
                H_[2, 2] = 0.0e0
    ...
    def eSIN211(self, nargout, *args):
        ...
        if nargout > 1:
            try:
                ...
                dim = len(EV_)
            g_ = np.zeros(dim)
            g_[0] = 2.0e0 * SINA * IV_[2] * IV_[1] * IV_[0]
            ...
            if nargout > 2:
                H_ = np.zeros((4, 4))
                H_[0, 0] = 2.0e0 * SINA * IV_[2] * IV_[1]
                H_[0, 1] = 2.0e0 * SINA * IV_[2] * IV_[0]
                H_[1, 0] = H_[0, 1]
                H_[0, 2] = 2.0e0 * SINA * IV_[1] * IV_[0]
                H_[2, 0] = H_[0, 2]
                H_[1, 2] = SINA * IV_[0] ** 2
                H_[2, 1] = H_[1, 2]
                H_[2, 2] = 0.0e0
                H_[3, 0] = 2.0e0 * COSA * IV_[2] * IV_[1] * IV_[0]
                H_[0, 3] = H_[3, 0]
    ...
    def eCOS211(self, nargout, *args):
        ...
        if nargout > 1:
            try:
                ...
                dim = len(EV_)
            g_ = np.zeros(dim)
            g_[0] = 2.0e0 * COSA * IV_[2] * IV_[1] * IV_[0]
            ...
            if nargout > 2:
                H_ = np.zeros((4, 4))
                H_[0, 0] = 2.0e0 * COSA * IV_[2] * IV_[1]
                H_[0, 1] = 2.0e0 * COSA * IV_[2] * IV_[0]
                H_[1, 0] = H_[0, 1]
                H_[0, 2] = 2.0e0 * COSA * IV_[1] * IV_[0]
                H_[2, 0] = H_[0, 2]
                H_[1, 2] = COSA * IV_[0] ** 2
                H_[2, 1] = H_[1, 2]
                H_[2, 2] = 0.0e0
                H_[3, 0] = -2.0e0 * SINA * IV_[2] * IV_[1] * IV_[0]
                H_[0, 3] = H_[3, 0]
    ...
    def eSIN31(self, nargout, *args):
        ...
        if nargout > 1:
            try:
                ...
                dim = len(EV_)
            g_ = np.zeros(dim)
            g_[0] = 3.0e0 * SINA * IV_[1] * IV_[0] ** 2
            g_[1] = SINA * IV_[0] ** 3
            g_[2] = COSA * IV_[1] * (IV_[0] ** 3)
            g_ = U_.T.dot(g_)
            if nargout > 2:
                H_ = np.zeros((3, 3))
                H_[0, 0] = 6.0e0 * SINA * IV_[1] * IV_[0]
                H_[0, 1] = 3.0e0 * SINA * IV_[0] ** 2
                H_[1, 0] = H_[0, 1]
                H_[2, 0] = 3.0e0 * COSA * IV_[1] * (IV_[0] ** 2)
                H_[0, 2] = H_[2, 0]
                H_[2, 1] = COSA * (IV_[0] ** 3)
                H_[1, 2] = H_[2, 1]
    ...
    def eCOS31(self, nargout, *args):
        ...
        if nargout > 1:
            try:
                ...
                dim = len(EV_)
            g_ = np.zeros(dim)
            g_[0] = 3.0e0 * COSA * IV_[1] * IV_[0] ** 2
            g_[1] = COSA * IV_[0] ** 3
            g_[2] = -SINA * IV_[1] * (IV_[0] ** 3)
            g_ = U_.T.dot(g_)
            if nargout > 2:
                H_ = np.zeros((3, 3))
                H_[0, 0] = 6.0e0 * COSA * IV_[1] * IV_[0]
                H_[0, 1] = 3.0e0 * COSA * IV_[0] ** 2
                H_[1, 0] = H_[0, 1]
                H_[2, 0] = -3.0e0 * SINA * IV_[1] * (IV_[0] ** 2)
                H_[0, 2] = H_[2, 0]
                H_[2, 1] = -SINA * (IV_[0] ** 3)
                H_[1, 2] = H_[2, 1]
