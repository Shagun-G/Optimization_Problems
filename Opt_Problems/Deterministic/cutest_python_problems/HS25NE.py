...
class HS25NE(CUTEst_problem):
    ...
    def __init__(self, *args):
        ...
        if "X1" in ix_:
            ...
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X1"]), float(100.0)
            )
        if "X2" in ix_:
            self.x0[ix_["X2"]] = float(12.5)
        else:
            self.y0 = arrset(
                self.y0, findfirst(self.congrps, lambda x: x == ig_["X2"]), float(12.5)
            )
        if "X3" in ix_:
    ...
    def eWFI(self, nargout, *args):
        ...
        if nargout > 1:
            ...
            if nargout > 2:
                ...
                H_[0, 1] = (
                    EXPO * EV_[2] * X2I * WMY ** (EV_[2] - 1.0) * (-1.0 + XI * WMYEZ)
                )
                H_[1, 0] = H_[0, 1]
                ...
                H_[1, 1] = (
                    ...
                    * WMY ** (EV_[2] - 2.0)
                    * EV_[2]
                    * (-EV_[2] + 1.0 + XI * EV_[2] * WMYEZ)
                )
                H_[1, 2] = (
                    EXPO
                    * XI
                    * WMY ** (EV_[2] - 1.0)
                    * (1.0 + EV_[2] * LWMY * (1.0 - XI * WMYEZ))
                )
                H_[2, 1] = H_[1, 2]
