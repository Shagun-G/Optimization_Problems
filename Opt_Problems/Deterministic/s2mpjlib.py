...
class CUTEst_problem:
    ...
    def LIHxyv(self, x, y, v, I):  # input = ( x, y, v, clist )
        ...
        self.getglobs()
        if hasattr(self, "congrps"):
            return self.evalLHxyv(
                self.objgrps, [self.congrps[i] for i in clist], x, y, v
            )
        else:
            return self.evalHJv("Hv", self.objgrps, x, v, [])
    ...
    def evalgrsum(self, isobj, glist, x, nargout):
        ...
        for iig in range(len(glist)):
            ...
            if (
                hasattr(self, "gconst")
                and ig < len(self.gconst)
                and not self.gconst[ig] is None
            ):
            ...
            if (
                hasattr(self, "grelt")
                and ig < len(self.grelt)
                and not self.grelt[ig] is None
            ):
            ...
            if (
                hasattr(self, "grftype")
                and ig < len(self.grftype)
                and not self.grftype[ig] is None
            ):
                ...
                if isobj:
                    if nargout == 1:
                        ...
                        if derlvl >= 2:
                            sgin = lil_matrix(gin)
                            Hx += (
                                Hessa * sgin.dot(sgin.transpose()) + grada * Hin
                            ) / gsc
                        else:
                            Hx = lil_matrix((n, n))
                            Hx[0, 0] = np.nan
                    ...
                    if nargout == 1:
                        ...
                        if derlvl >= 2:
                            Hx.append(
                                (Hessa * sgin.dot(sgin.transpose()) + grada * Hin) / gsc
                            )
                        else:
                            Hxi = lil_matrix((n, n))
                            Hxi[0, 0] = np.nan
        ...
        for iig in range(len(glist)):
            ...
            if mode == "Hv":
                if egname == "TRIVIAL":
                    ...
                    sgin = lil_matrix(gin)
                    HJv += (
                        (Hessa * sgin) * (sgin.transpose().dot(v)) + grada * Hinv
                    ) / gsc
            elif mode == "HIv":
                if egname == "TRIVIAL":
                    ic += 1
...
def s2mpj_nlx(self, name, List, getxnames=None, xlowdef=None, xuppdef=None, x0def=None):
    ...
    if newvar:
        ...
        if hasattr(self, "xlower"):
            ...
            if iv <= thelen:
                self.xlower = np.append(
                    self.xlower, np.full((iv - thelen + 1, 1), float(0.0)), 0
                )
        ...
        if hasattr(self, "xupper"):
            ...
            if iv <= thelen:
                self.xupper = np.append(
                    self.xupper, np.full((iv - thelen + 1, 1), float("Inf")), 0
                )
...
def s2mpjlib_select(classif, *args):
    ...
    if classif in ["help", "h"]:
        ...
        print(
            "    N the problem’s constraints represent the adjacency matrix of a (linear)"
        )
        print("      network;")
        ...
        print(
            " The fourth character after the first hyphen indicates the smoothness of"
        )
        print(" the problem. There are two possible choices")
        print(
            "    R the problem is regular, that is, its first and second derivatives "
        )
        print("      exist and are continuous everywhere; or")
        print("    I the problem is irregular.")
        print(
            " The integer (r) which corresponds to the fourth character of the string is"
        )
        print(
            " the degree of the highest derivatives provided analytically within the problem"
        )
        print(
            " description. It is restricted to being one of the single characters O, 1, or 2."
        )
        print(
            " The character immediately following the second hyphen indicates the primary"
        )
        print(" origin and/or interest of the problem. Its possible values are")
        print(
            "    A the problem is academic, that is, has been constructed specifically by"
        )
        print("      researchers to test one or more algorithms;")
        print(
            "    M the problem is part of a modeling exercise where the actual value of the"
        )
        ...
        print(
            " description contains explicit internal variables. There are two possible"
        )
        print(" values, namely,")
        ...
        print(
            " The symbol(s) between the third and fourth hyphen indicate the number of"
        )
        ...
        print(
            " The symbol(s) after the fourth hyphen indicate the number of constraints"
        )
        print(
            " (other than fixed variables and bounds) in the problem. Note that fixed"
        )
        print(
            " variables are not considered as general constraints here. The two possible"
        )
        print(" values are")
        print(
        ...
        print("  ")
        print(
            " In order to use the selection too, you should first open Python in the parent"
        )
        print(
            " of the directory containing the Python problem files, then import the library"
        )
        print(" by issuing the command")
        print("    from Opt_Problems.Deterministic.s2mpjlib import *")
        print(" or, more specifiaclly,")
        print("    from s2mpjlib import s2mpjlib_select")
        print(
            " The select tool may then be called with its first argument being a string "
        )
        print(
            " which specifies the class of problems of interest.  This string is constructed"
        )
        print(
            " by replacing by a dot each character in the classification string for which"
        )
        print(" all possible values are acceptable (the dot is a wildcard character).")
        print(" For instance")
        print('    s2mpjlib_select( "C-CSU..-..-2-0" ) ')
        print(
            " lists all CUTEst unconstrained "
            "sum-of-squares"
            " problems in two continuous"
        )
        print(" variables, while ")
        print("    s2mpjlib_select( " "C-C....-..-V-V" " ) ")
        print(
            " lists all CUTEst problems with variable number of continuous variables and"
        )
        ...
        print(
            " NOTE: any regular expression may be used as the first argument of select "
        )
        print(
            "       to specify the problem class, so that, for instance, the previous "
        )
        print('       selection can also be achieved by s2mpjlib_select( "C-C.*V-V" ) ')
        print(
            " Writing the list of selected problems to a file is obtained by specifying"
        )
