...
class Cross_Entropy_Binary(Unconstrained):
    ...
    def __init__(self, location : str, name : str | None = None, sparse_format : bool = False) -> None:
        ...
        object attributes:
        number_of_features : includes 1 of bias term
        X : feature dataset (number of features x number of datapoints), includes bias term constant
        Y : target labels ({0,1} x number of datapoints)
        '''

        self._sparse = sparse_format
        self._name = f"{name}_cross_entropy_logistic"
        ...
        if name.lower() == "mushroom":
            ...
            # the has to be changed from {-1, +1} to {0, 1}
            y[y == -1] = 0
        else:
        ...
        self._features = X
        self._targets = y
        super().__init__(name = self.name, d = self._number_of_features)
